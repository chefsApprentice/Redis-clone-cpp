...
static auto try_one_request(Conn* conn) -> bool {
    // try to parse the protocol: message header
    if (buf_size(&conn->incoming) < 4) {
        return false; // want read
    }
    uint32_t len = 0;
    memcpy(&len, conn->incoming.data_start, 4);
    if (len > k_max_msg) {
        msg("too long");
    ...
    const uint8_t* request = conn->incoming.data_start + read_size;
...
static void handle_write(Conn* conn) {
    ...
    ssize_t rv = write(conn->fd, conn->outgoing.data_start, buf_size(&conn->outgoing));
...
auto main() -> int {
    ...
    while (true) {
        ...
        for (size_t i = 1; i < poll_args.size(); ++i) { // note: skip the 1st
            ...
            if (((ready & POLLERR) != 0U) || conn->want_close) {
                conn_destroy(conn);
            }
